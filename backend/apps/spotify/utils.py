...
def update_or_create_user_tokens(user, access_token, expires_in, refresh_token):
    ...
    if tokens:
        ...
        tokens.save(update_fields=['access_token', 'refresh_token', 'expires_at'])
    else:
        tokens = SpotifyToken(user=user, access_token=access_token, refresh_token=refresh_token, expires_at=expires_at)
        tokens.save()
