...
- **On-the-fly metric computation:** Jaccard diversity iterates over the 50 most recently rated tracks (capped in `views.py`). Worst case is 50×49/2 = 1,225 pairs — negligible. If the cap were removed, complexity would be O(N²) in the number of tracks with non-empty genre data.
